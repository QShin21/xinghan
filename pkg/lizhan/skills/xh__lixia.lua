...
Fk:loadTranslationTable {
  ...
  ["xh__lixia_drawcards"] = "令 %src 摸一张牌",
  ["xh__lixia_loseHp"] = "弃置 %src 装备区内的一张牌并失去1点体力",

  ["$xh__lixia1"] = "将军真乃国之栋梁。",
  ["$xh__lixia2"] = "英雄可安身立命于交州之地。",
}
...
lixia:addEffect(fk.EventPhaseStart, {
  ...
  can_trigger = function(self, event, target, player, data)
    return player:hasSkill(lixia.name) and target ~= player and target.phase == Player.Start and
      not target.dead and not target:inMyAttackRange(player)
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    local choices = {}
    table.insert(choices, "xh__lixia_drawcards:" .. player.id)
    if not target.dead and #player:getCardIds("e") > 0 then
      table.insert(choices, "xh__lixia_loseHp:" .. player.id)
    end
    local choice = room:askToChoice(target, { choices = choices, skill_name = lixia.name })
    if choice == "xh__lixia_drawcards:" .. player.id then
      player:drawCards(1, lixia.name)
    elseif choice == "xh__lixia_loseHp:" .. player.id then
