...
fudi:addEffect(fk.Damaged, {
  ...
  can_trigger = function(self, event, target, player, data)
    if target ~= player or not player:hasSkill(xh__fudi.name) then return false end
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:moveCardTo(id, Player.Hand, source, fk.ReasonGive, xh__fudi.name, nil, false, player.id)
